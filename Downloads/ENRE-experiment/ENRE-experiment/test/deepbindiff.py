...
def cal_block_embeddings(blockIdxToTokens, tokenEmbeddings, reversed_dictionary):
    block_embeddings = {}
        ...
        if len(tokenlist) != 0:
            for token in tokenlist:
                ...
                if tokenid in opcode_idx_list:
                    opcodeEmbeddings.append(tokenEmbedding)
                else:
...
def main():
    ...
    # step 0: ground truth collection
    gtc.gtc(filepath1, old_dir, filepath2, new_dir, outputDir)

    # step 1: perform preprocessing for the two binaries
    blockIdxToTokens, _, _, bin1_name, bin2_name, toBeMergedBlocks = preprocessing.preprocessing(filepath1, filepath2, outputDir)
    ...
    block_embeddings = cal_block_embeddings(blockIdxToTokens, tokenEmbeddings, reversed_dictionary)
    ...
    two_lv_graph_match.two_level_matching(bin1_name, bin2_name, toBeMergedBlocks)
